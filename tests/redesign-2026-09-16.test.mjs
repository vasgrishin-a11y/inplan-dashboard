...
test('KPI-карточки одной структуры, подсказка — только на значке «?»', async (t) => {
  ...
  assert.ok(checked >= 39, `проверено карточек: ${checked}`);
  assert.ok(withHelp / checked > 0.9, `подсказка есть почти у всех расчётных карточек (${withHelp}/${checked})`);
});

test('«Спрос и покрытие»: бездефицитные заказы перенесены в подпись «Отгружено»', async (t) => {
  const ctx = await loadApp();
  t.after(ctx.close);
  await ctx.go('dm', 180);

  assert.equal(ctx.kpi('Заказов без дефицита'), undefined,
    'отдельной карточки «Заказов без дефицита» больше нет');
  const shipped = ctx.kpi('Отгружено');
  assert.ok(shipped, 'карточка «Отгружено» осталась');
  assert.match(shipped.querySelector('.s').textContent, /заказов без дефицита/i,
    'информация о заказах без дефицита показана в мелкой подписи «Отгружено»');
});
